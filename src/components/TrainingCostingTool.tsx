...
const TrainingCostingTool: React.FC<TrainingCostingToolProps> = ({ 
  ...
  const [additionalLocations, setAdditionalLocations] = useState<TrainingLocation[]>([]);
  const [apiBaseUrl, setApiBaseUrl] = useState<string>('');
  
  console.log('TrainingCostingTool received initialData:', initialData);
  
  const { register, watch, control, setValue, handleSubmit, formState: { errors }, trigger, getValues } = useForm<TrainingCost>({
    defaultValues: {
      ...
      costMode: initialData?.costMode || 'perdiem',
  ...
  useEffect(() => {
    if (initialData) {
      console.log('Initializing TrainingCostingTool with data:', initialData);
      ...
      if (initialData.trainings && Array.isArray(initialData.trainings)) {
        setAdditionalLocations(initialData.trainings);
      }
      ...
      setValue('trainingLocation', initialData.trainingLocation || '');
      setValue('costMode', initialData.costMode || 'perdiem');
  ...
  useEffect(() => {
    ...
    console.log('API Base URL for training tool:', apiUrl);
  }, []);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      setError(null);
      console.log('Fetching training costing data...');
      ...
      try {
        ...
        if (!locationsResult?.data || !Array.isArray(locationsResult.data)) {
          console.error('Invalid locations data received in TrainingCostingTool:', 
        ...
        console.error('Error fetching training costing data:', error);
  ...
  const updateTrainingLocation = (index: number, field: keyof TrainingLocation, value: any) => {
    const newLocations = [...additionalLocations];
    newLocations[index] = {
      ...newLocations[index],
      [field]: value
  ...
  useEffect(() => {
    const calculateTotalBudget = () => {
      ...
      const numSessions = Number(watchSessions) || 1; // Number of training sessions
      ...
      if (costMode === 'perdiem') {
        ...
        if (perDiem) {
          ...
          accommodationTotal = (perDiemAmount + hardshipAmount) * participants * days;
          ...
          additionalLocations.forEach(loc => {
            ...
            if (additionalPerDiem) {
              ...
              accommodationTotal += (addPerDiemAmount + addHardshipAmount) * loc.participants * loc.days;
            }
          });
        }
      } else {
        ...
        if (days > 1) {
          ...
          if (accommodation) {
            accommodationTotal = Number(accommodation.price) * participants * (days - 1);
            ...
            additionalLocations.forEach(loc => {
              ...
              if (additionalAccommodation) {
                accommodationTotal += Number(additionalAccommodation.price) * loc.participants * (loc.days - 1);
              }
            });
          }
        }
      ...
      if (watchParticipantCosts) {
        if (watchParticipantCosts.includes('ALL')) {
          // Get sum of all participant costs except ALL
          ...
          // Apply costs to participants in additional locations
          additionalLocations.forEach(loc => {
            if (!loc.locationId) return;
      ...
      if (watchSessionCosts) {
        if (watchSessionCosts.includes('ALL')) {
          // Get sum of all session costs except ALL
    ...
    watchLocation, watchDays, watchParticipants, watchSessions, watchCostMode, watchTransportRequired,
  ...
  const handleFormSubmit = async (data: TrainingCost) => {
    try {
      ...
      console.log('Training calculated budget:', calculatedBudget);
      ...
      if (watchTransportRequired) {
        // Count participants and costs from land routes
      ...
      const budgetData = {
        ...
        activity_type: 'Training', // Explicitly set type
        ...
        training_details: {
          ...
          trainings: additionalLocations.map(loc => ({
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6 max-h-[75vh] overflow-y-auto p-2 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">
            Number of times this training will occur
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            {...register('costMode', { required: 'Cost mode is required' })}
            className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
          >
          ...
          <p className="mt-1 text-xs text-gray-500">
            {watchCostMode === 'perdiem' 
              ? 'Per diem covers daily allowance for participants'
              : 'Accommodation covers lodging costs (excludes first day)'}
          </p>
        </div>
      </div>
      ...
      <div className="mt-4">
        <div className="flex justify-between items-center mb-4">
          <label className="block text-sm font-medium text-gray-700">
            Additional Training Locations (optional)
          </label>
          <button
            type="button"
      ...
      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
        ...
        <p className="mt-2 text-sm text-gray-500 flex items-center">
          ...
          This total includes {watchCostMode === 'perdiem' ? 'per diem' : 'accommodation'} costs
